...
void usb_arch_setup_control_endpoint(unsigned char addr)
{
...
void usb_arch_setup_bulk_endpoint(unsigned char addr)
{
	USB_HW_Buffer *buf_desc;
	unsigned int ei = EP_HW_NUM(addr);
	unsigned int epr;
...
void usb_arch_setup_interrupt_endpoint(unsigned char addr)
{
	USB_HW_Buffer *buf_desc;
	unsigned int ei = EP_HW_NUM(addr);
	unsigned int epr;
...
static int handle_pending_receive(USBEndpoint *ep)
{
	...
	unsigned int copy;
	unsigned int res = 0;
	volatile unsigned int hw_offset;
	...
	if (!(flags & USB_EP_FLAGS_ENABLED) || !buffer)
	{
	...
	switch(flags & USB_EP_FLAGS_TYPE_MASK)
	{
		case USB_EP_FLAGS_TYPE_CONTROL:
			...
			if (flags & USB_EP_FLAGS_SETUP_PENDING)
			{
				...
				buffer = skip_buffers_until(buffer, USB_BUFFER_SETUP, USB_BUFFER_SETUP, &res);
				ep->buffer = buffer;
...
static int start_transmit(USBEndpoint *ep)
{
	...
	if (!(ep_flags & USB_EP_FLAGS_ENABLED) || !buffer)
	{
	...
	switch(ep_flags & USB_EP_FLAGS_TYPE_MASK)
	{
		case USB_EP_FLAGS_TYPE_CONTROL:
			...
			if (get_transmit_length(ep->buffer) <= len)
			{
				...
				USB->EPR[hw_ep] = USB_EPxR_EP_STATUS_OUT | EPR_TOGGLE_SET(epr, USB_EP0R_STAT_RX, USB_EPxR_STAT_RX_NAK);
			}
			else
			{
				USB->EPR[hw_ep] = USB_EPxR_EP_STATUS_OUT | EPR_TOGGLE_SET(epr, USB_EP0R_STAT_RX, USB_EPxR_STAT_RX_STALL);
			}
...
void usb_arch_discard_all_buffers(unsigned char ep_addr)
{
  uint32_t ints;
  USBBuffer *buffer;
...
void usb_arch_control_stall(unsigned char addr)
{
...
void usb_arch_halt_endpoint(unsigned char ep_addr, int halt)
{
	if (EP_INDEX(ep_addr) > USB_MAX_ENDPOINTS)
	{
		return;
	}

	if (!(usb_endpoints[EP_INDEX(ep_addr)].flags & USB_EP_FLAGS_ENABLED))
	{
		return;
	}
...
void usb_arch_set_address(unsigned char addr)
{
  USB->DADDR = 0x80 | addr;  
}
